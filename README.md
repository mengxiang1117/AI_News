...
基于大模型的财经新闻精准推送系统。自动从同花顺 6 大新闻源 7×24 小时并行抓取最新资讯，通过 AI 智能识别新闻所属领域、分析市场情绪（利好/利空/中性），按用户兴趣精准匹配并实时推送到飞书群。

**核心价值**：让你在海量财经新闻中，只看到真正关心的内容，第一时间掌握市场动向。

## 功能特点

- **6大新闻源**：7×24小时要闻、原创情报、涨停解密、快评、公司互动、公告解读
- **并行爬取**：多线程同时抓取多个新闻源，提升效率
- **AI 智能分析**：一次 API 调用同时完成领域识别和情绪分析，速度快
- **情绪分析**：大模型判断新闻对市场的影响方向（利好/利空/中性）
...
```
爬取新闻 → AI分析领域 → AI情绪分析 → 匹配用户兴趣 → 推送飞书
    ↓           ↓              ↓             ↓
  并行抓取    并发调用API    判断利好利空    按用户配置过滤
```
...
| 情绪 | 标识 | 说明 |
|------|------|------|
| 利好 | 🔴 | 业绩增长、政策扶持、技术突破等正面消息 |
| 利空 | 🟢 | 业绩下滑、监管处罚、安全事故等负面消息 |
| 中性 | ⚪ | 常规公告、影响不明确的消息 |

情绪分析结果会显示在飞书推送消息中，帮助用户快速判断新闻影响。
...
粗略统计24小时，使用500WTokens。

### Q: 智能识别新闻所属领域的判断依据是什么？

大模型根据以下规则判断：
- **穿透个股看行业**：识别股票代码或公司名称背后的主营业务，映射到所属行业领域
- **核心事实优先**：依据新闻的核心内容和实质影响分类，而非偶然提及的实体
- **严格闭域分类**：只能从配置的领域列表中选择，不会臆造新领域
- **多领域提取**：涉及交叉领域时，提取所有核心相关领域

如需调整领域列表，可修改 `config.yaml` 中的 `all_categories` 配置。

### Q: 如何自定义领域识别和情绪分析的提示词？

编辑 `prompt.py` 文件中的 `NEWS_ANALYSIS` 模板即可。可调整的内容包括：
- **领域分析规则**：修改领域判断的核心逻辑和权重
- **情绪判断标准**：调整利好/利空/中性的判断依据
- **输出格式**：改变返回结果的结构

### Q: 情绪分析的判断依据是什么？

大模型根据以下标准判断：
- **利好**：业绩超预期、获得订单/合作、政策扶持、技术突破、机构增持
- **利空**：业绩下滑、政策限制、重大诉讼/处罚、产品问题、机构减持
- **中性**：常规公告、信息不明确、正负因素相互抵消

如需调整判断标准，可修改 `prompt.py` 中的 `NEWS_ANALYSIS` 模板。

### Q: 如何提升处理速度？

系统已优化为单次 API 调用同时完成领域识别和情绪分析。如需进一步提速：
- **增加并发数**：修改 `config.yaml` 中的 `max_workers`（默认3，可提升到5-10）
- **使用更快模型**：将 `gpt-4` 改为 `gpt-3.5-turbo`
- **精简新闻内容**：减少每次分析的 token 数量
