...
class NewsPusher:
    ...
    def extract_news_categories(self, news: Dict) -> List[str]:
        """
        使用大模型提取新闻相关的领域标签（支持多模型顺序尝试）
        ...
        prompt = get_news_category_prompt(categories, news['title'], news['content'])

        last_error = None
        for model in models:
            try:
                logger.debug(f"尝试使用模型: {model}")
                response = self.openai_client.chat.completions.create(
                    model=model,
                    messages=[
                        {"role": "system", "content": "你是一个专业的财经新闻分析师，擅长识别新闻所属的领域。"},
                ...
                if result_text in ("[]", "空", "无", "无相关领域", "None", "none", "null"):
                    logger.debug(f"模型 {model} 判断新闻无股票相关领域")
                    return []

                # 提取方括号内的内容
                json_match = re.search(r'\[.*\]', result_text, re.DOTALL)
                if json_match:
                    news_categories = json.loads(json_match.group())
                else:
                    # 尝试直接解析
                    news_categories = json.loads(result_text)

                if not isinstance(news_categories, list):
                    logger.warning(f"模型 {model} 返回结果不是数组: {result_text}")
                    return []
